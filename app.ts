    ...
    app.post('/upload', upload.single('file'), async (req, res) => {
      try {
        ...
        const fileSizeLimit = 3 * 1024; // 3KB
      ...
      pool.query(createTableQuery)
        ...
        const insertQuery = `
          ...
          for (const row of data) {
            ...
            await pool.query(insertQuery, sanitizedRow); // Insert each row into the database
          }
      
          // Remove the uploaded file
          await fs.promises.unlink(filePath);
      
          res.status(200).json({ message: 'File uploaded and inserted into the database successfully' });
        } catch (error) {
          console.error(error);
